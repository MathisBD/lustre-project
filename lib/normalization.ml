...
let new_local =
  ...
  fun () ->
    incr cpt;
    "aux'" ^ string_of_int !cpt

(** [new_pat e] prend en argument une expression [e] et retourne une variable
    (ou une liste de variable si [e] est un tuple) du même type que [e].

    Plus précisément, cette fonction retourne un triplet [(decl, patt, expr)],
    où [decl] est la déclaration de la variable (ou une liste de déclaration si
    [e] est un tuple), [patt] est la variable (ou le tuple de variables) vue
    comme un motif et [expr] est la variable (ou le tuple de variables) vue
...
(** [normalize ctx e] met l'expression [e] en forme normale (en <bexpr>) et
    ajoute à [ctx] les équations introduites lors de la normalisation. *)
let rec normalize ctx e =
  ...
  | TE_binop (_op, _e1, _e2) -> (ctx, e (* TODO *))
  | TE_app (n, le) ->
  ...
  | TE_print le ->
      ...
      let x_decl, x_patt, x_expr = new_pat e in
      let x_eq =
        { teq_patt = x_patt; teq_expr = { e with texpr_desc = TE_print le' } }
      in
      ((x_decl @ new_vars, x_eq :: new_eqs), x_expr)
  | TE_if (e1, e2, e3) ->
      let ctx, e1' = normalize ctx e1 in
