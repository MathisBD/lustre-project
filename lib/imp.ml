...
let rec untulify el =
  List.fold_right
    (fun e acc ->
      match e.texpr_desc with TE_tuple el -> untulify el @ acc | _ -> e :: acc)
    el []
...
let rec compile_base_expr e =
  let desc =
    ...
    | TE_binop (op, e1, e2) ->
        ...
        ME_binop (op, ce1, ce2)
    | TE_if (_e1, _e2, _e3) -> assert false (* TODO *)
    | TE_tuple el -> ME_tuple (List.map compile_base_expr el)
    | TE_print el -> ME_print (List.map compile_base_expr el)
    | TE_pre _ | TE_arrow _ | TE_app _ ->
        assert false (* impossible car en forme normale *)
    | TE_prim (f, el) ->
        let _, f_out_ty =
...
let compile_equation { teq_patt = p; teq_expr = e }
    ( (mem_acc : Imp_ast.mem)
  ...
  match e.texpr_desc with
  | TE_pre _e1 -> (mem_acc, init_acc, compute_acc, update_acc (* TODO *))
  | TE_arrow (_e1, _e2) ->
      (mem_acc, init_acc, compute_acc, update_acc (* TODO *))
  | TE_app (_n, _el) -> (mem_acc, init_acc, compute_acc, update_acc (* TODO *))
  | _ ->
...
let rename_equation env eq = { eq with meq_expr = rename_expr env eq.meq_expr }
...
let rename_node env n =
  ...
  let init =
    { n.mn_init with
      node_init =
        List.map (fun (x, f) -> (x, List.assoc f env)) n.mn_init.node_init
    }
  in
  let compute = List.map (rename_equation env) n.mn_compute in
  ( (n.mn_name, id) :: env
  , { n with mn_name = id; mn_mem = mem; mn_init = init; mn_compute = compute }
  )
...
let rename_nodes f main =
  let env, f' =
    List.fold_left
      (fun (env, f) n ->
        let env', n' = rename_node env n in
        (env', n' :: f))
      ([], []) f
  in
