...
(** Enregistrement représentant la déclaration de la mémoire interne d'un noeud.
*)
type mem =
  { fby_mem : tvar list
        (** liste des mémoires associées aux fby. Par exemple, la valeur
            [ [("next_x", Tbool)] ] correspond au code généré
            [ "next_x : bool;" ] *)
  ; node_mem : (string * string) list
        (** liste des mémoires associées aux appels de noeuds Par exemple, la
            valeur [ [("mem1", "f")] ] correspond au code généré
            [ "mem1 : f_mem;" ] *)
  }
...
type init =
  { fby_init : (string * const) list
        (** initialisation des fby Par exemple, la valeur
            [ [("next_x", Cbool false)] ] correspond au code généré
            [ "next_x = false;" ] *)
  ; node_init : (string * string) list
        (** initialisation des mémoires associées aux appels de noeuds Par
            exemple, la valeur [ [("mem1", "f")] ] correspond au code généré
            [ "mem1 = f_init();" ] *)
  }

type m_expr = { mexpr_desc : m_expr_desc; mexpr_type : base_ty list }
...
type m_equation = { meq_patt : tvar list; meq_expr : m_expr }
...
type m_node =
  ...
  ; mn_init : init  (** initialisation de la mémoire du noeud *)
