...
let string_of_list =
  ...
  in
  fun f sep l -> string_of_list "" f sep l
...
let string_of_unop op =
  match op with Unot -> "not" | Uminus -> "-" | Uminus_f -> "-."
...
let string_of_const c =
  ...
  | Cint i -> if i < 0 then "(" ^ string_of_int i ^ ")" else string_of_int i
  | Cfloat f ->
      if f < 0.0 then "(" ^ string_of_float f ^ ")" else string_of_float f
  | Cstring s -> "\"" ^ s ^ "\""

let string_of_atom a =
  match a with Const c -> string_of_const c | Ident x -> x
...
let rec string_of_expr e =
  ...
  | ME_binop (op, e1, e2) ->
      sprintf "(%s %s %s)" (string_of_expr e1) (string_of_binop op)
        (string_of_expr e2)
  | ME_if (e1, e2, e3) ->
      sprintf "@\n    (if %s then@\n       %s@\n     else@\n       %s)"
        (string_of_expr e1) (string_of_expr e2) (string_of_expr e3)
  | ME_tuple el -> sprintf "(%s)" (string_of_list string_of_expr ", " el)
  | ME_app (f, mem, args) ->
      sprintf "(%s_step mem'.%s (%s))" f mem
        (string_of_list string_of_expr ", " args)
  | ME_prim (f, args, _) ->
...
let print_mem n =
  if n.mn_mem = { fby_mem = []; node_mem = [] }
  then printf "type %s_mem = unit@\n" n.mn_name
  else begin
    printf "type %s_mem = {@\n" n.mn_name;
    print_list
...
let print_update (x, a) = printf "  mem'.%s <- %s;@\n" x (string_of_atom a)
